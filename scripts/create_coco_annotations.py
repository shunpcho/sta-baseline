...
with Path(args.path_to_annotations).open(encoding="utf-8") as f:
    labels = json.load(f)
...
info = {
    ...
    "version": "1.0",
    "date_created": str(datetime.datetime.now(tz=datetime.timezone(datetime.timedelta(hours=9))).date()),  # JST
}

categories = [{"supercategory": "object", "id": k, "name": v} for k, v in idx_to_obj.items()]
...
coco_annotations = {"info": info, "images": images, "annotations": annotations, "categories": categories}

with Path(args.path_to_output_json).open("w", encoding="utf-8") as f:
