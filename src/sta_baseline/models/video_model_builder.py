...
from sta_baseline.models import head_helper, resnet_helper, stem_helper
from sta_baseline.models.batchnorm_helper import get_norm
from sta_baseline.models.build import MODEL_REGISTRY
from sta_baseline.utils import weight_init_helper as init_helper
...
class MViT(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_abs_pos:
            if self.sep_pos_embed:
                pos_embed = self.pos_embed_spatial.repeat(1, self.patch_dims[0], 1) + torch.repeat_interleave(
...
class MultiScaleBlock(nn.Module):
    def __init__(
        ...
        self.pool_skip = (
            nn.MaxPool3d(kernel_skip, stride_skip, padding_skip, ceil_mode=False) if len(kernel_skip) > 0 else None
        )
...
def cal_rel_pos_temporal(attn, q, has_cls_embed, q_shape, k_shape, rel_pos_t):
    ...
    if q.dim() == 4 and attn.dim() == 4:
        ...
        attn[:, sp_idx:, sp_idx:] = (
            attn[:, sp_idx:, sp_idx:].view(B_n_head, q_t, q_h, q_w, k_t, k_h, k_w) + rel[:, :, :, :, :, None, None]
        ).view(B_n_head, q_t * q_h * q_w, k_t * k_h * k_w)
    else:
        raise NotImplementedError(f"Unsupported q shape {q.shape}")

    return attn  # !/usr/bin/env python3
