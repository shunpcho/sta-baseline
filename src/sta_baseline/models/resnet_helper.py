...
def get_trans_func(name):
    ...
    assert name in trans_funcs, f"Transformation function '{name}' not supported"
...
class ResStage(nn.Module):
    ...
    def _construct(
        ...
        for pathway in range(self.num_pathways):
            for i in range(self.num_blocks[pathway]):
                ...
                self.add_module(f"pathway{pathway}_res{i}", res_block)
    ...
    def forward(self, inputs):
        ...
        for pathway in range(self.num_pathways):
            x = inputs[pathway]
            for i in range(self.num_blocks[pathway]):
                m = getattr(self, f"pathway{pathway}_res{i}")
                x = m(x)
                if hasattr(self, f"pathway{pathway}_nonlocal{i}"):
                    nln = getattr(self, f"pathway{pathway}_nonlocal{i}")
                    b, c, t, h, w = x.shape
